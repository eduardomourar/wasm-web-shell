...
#[cfg(test)]
mod test {
    use super::{DefaultClient, Headers, Request};
    ...
    pub fn build_headers() {
        ...
        input_headers.insert(http::header::HOST, "example.com".parse().unwrap());
        input_headers.insert(http::header::CONTENT_LENGTH, "123".parse().unwrap());

        let header_id = Headers::from(&input_headers).to_owned();
        assert_ne!(header_id, 0);
        let headers = http_types::fields_entries(header_id);
        assert!(headers
            .iter()
            .any(|(k, v)| *k == http::header::HOST.to_string() && *v == "example.com".as_bytes()));
        assert!(
            headers
                .iter()
                .any(|(k, v)| *k == http::header::CONTENT_LENGTH.to_string()
                    && *v == "123".as_bytes())
        );
    }
    ...
    pub fn build_request() {
        let http_request = http::Request::builder()
            ...
            .header("User-Agent", "wasi-http/0.2.0")
            .body(Bytes::new())
            .unwrap();
    ...
    pub fn handle_request() {
        ...
        let response = client.handle(request).unwrap();
